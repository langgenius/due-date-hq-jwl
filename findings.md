...
- **F-015 · EmptyState adoption — RECLASSIFIED, non-issue** — inspected every "No X" candidate on non-alerts surfaces. The surfaces that warrant an empty-state *card* already use `<EmptyState>` (31 importers; audit-log, sources-tab, notes, work-plan, facts, etc. — the grep matched the copy *inside their `title` prop*). The remaining hand-rolled "No X" strings are correctly **inline micro-empties** — dropdown-menu items, table cells, one-line `text-tertiary` panel slots, status labels — which should NOT become cards (overkill, breaks compact layouts). The "~27% adoption" metric counted every empty string, not just card-pattern surfaces. **Nothing to fix; converting would be net-negative.**
