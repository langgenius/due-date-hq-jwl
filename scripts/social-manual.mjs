...
const HTML_OUT = path.join(ROOT, 'docs/marketing/xiaohongshu', LIGHT ? 'card-template-light.html' : 'card-template.html')
...
const mdY = (iso) => { const [ , m, d ] = iso.split('-'); return `${+m}月${+d}日` }

// ---------- 1. 计算权威数字(与 X 同源)----------
const archive = JSON.parse(fs.readFileSync(path.join(ROOT, 'apps/marketing/src/lib/disaster-archive.json'), 'utf8'))
const notices = (() => {
...
const ts = archive.map((x) => new Date(x.issuedOn).getTime()).filter((t) => !isNaN(t) && new Date(t).getFullYear() >= 2020).sort((a, b) => a - b)
...
const groups = Object.keys(byDate).sort().map((d) => ({ date: mdY(d), states: [...byDate[d]].map(cn).join(' · ') }))
// 封面 3 例：截止日最近的 3 个不同州
const coverChips = [...notices].sort((a, b) => a.deadline.localeCompare(b.deadline))
  .filter((n, i, arr) => arr.findIndex((m) => m.abbr === n.abbr) === i).slice(0, 3)
  .map((n) => ({ s: cn(n.abbr), d: mdY(n.deadline) }))

const data = { total, peakYear, peakCount, thisYear, thisYearCount, avgDays, activeCount, stateCount, terrCount, groups, coverChips }
...
const wm = (ink, bar) => `<svg class="wm" width="112" height="15" viewBox="0 0 1165 154" role="img" aria-label="DueDateHQ">${wmInner.replaceAll('#1F315C', ink).replaceAll('#F2F4ED', bar)}</svg>`

function buildHtml(d) {
  const T = LIGHT
    ? { bg:'#f3f2ec', ink:'#1e2138', sub:'#6b6e82', mut:'#a7a494', accent:'#2e368c', amber:'#c8791a', num:'#2e368c', wm:wm('#2e368c','#f3f2ec'), title:'#2e368c', big:'#2e368c', bigU:'#c8791a' }
    : { bg:'#2e368c', ink:'#ffffff', sub:'#c2c6ee', mut:'#8f95cf', accent:'#14c5f6', amber:'#ffbf4d', num:'#ffffff', wm:wm('#f2f4ee','#2e368c'), title:'#ffffff', big:'#ffffff', bigU:'#14c5f6' }
  const CARD = (pg, body) => `<div class="frame"><div class="card"><div class="top">${T.wm}<span class="pg num">${pg}</span></div>${body}</div></div>`
  const css = `<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  const chips = d.coverChips.map((c) => `<div class="de"><div class="s">${c.s}</div><div class="d">${c.d}</div></div>`).join('')
  const stgs = d.groups.map((g) => `<div class="stg"><span class="dt">${g.date}</span><span class="nm">${g.states}</span></div>`).join('')
  const cards = [
    CARD('1 / 6', `<div class="kicker">${d.thisYear} 才过半</div><div class="headline">IRS 已经发了这么多份<br/>灾害延期通知</div><div class="bignum num"><b>${d.thisYearCount}</b><span>份</span></div><div class="proof"><div class="lab">这些州的报税截止日，已被推迟到</div><div class="dates num">${chips}</div><div class="cover-foot"><span>美国报税不漏DDL</span><span class="peek">数据在图2 →</span></div></div>`),
    CARD('2 / 6', `<h2 class="title">IRS 灾害延期通知，到底有多频繁？</h2><div class="rows"><div class="row"><span class="data-num num">${d.total}</span><span class="data-label"><strong>${years0(d)} 至今</strong>发出的延期通知总数</span></div><div class="row"><span class="data-num num">${d.avgDays}<u>天</u></span><span class="data-label">平均每 ${d.avgDays} 天，<strong>就有一次</strong>新的延期</span></div><div class="row peak"><span class="data-num num">${d.peakCount}</span><span class="data-label"><strong>${d.peakYear} 全年</strong>，<em>历史峰值</em></span></div><div class="row"><span class="data-num num">${d.activeCount}</span><span class="data-label">份延期通知仍在生效，覆盖 <strong>${d.stateCount} 州 + ${d.terrCount} 地区</strong></span></div></div><div class="src">数据来自 DueDateHQ 核实的 IRS 灾害延期档案（irs.gov 逐条核对）</div>`),
    CARD('3 / 6', `<h2 class="title">此刻，<em>这些州和地区</em>的截止日正在延后</h2><div class="states num">${stgs}</div><div class="src">截至今日在生效；同一州可能含多份通知，均按 IRS 官方划定的受灾地区生效</div>`),
    CARD('4 / 6', `<h2 class="title">灾害延期，<em>覆盖哪些申报？</em></h2><div class="cov"><div class="ci"><span class="n">个人所得税</span><span class="f">1040</span></div><div class="ci"><span class="n">公司</span><span class="f">1120 / 1120-S</span></div><div class="ci"><span class="n">合伙企业</span><span class="f">1065</span></div><div class="ci"><span class="n">遗产与信托</span><span class="f">1041</span></div><div class="ci"><span class="n">工资/就业税</span><span class="f">941 / 940</span></div><div class="ci"><span class="n">季度预估税款</span><span class="f">窗口期内到期的</span></div></div><div class="src">上列为主要类型；延期窗口内到期的申报与缴款一并顺延，具体以 IRS 官方通知为准</div>`),
    CARD('5 / 6', `<h2 class="title">3 个<em>容易忽略</em>的点</h2><div class="pts"><div class="pt"><span class="no num">01</span><div><div class="h">延期自动生效</div><div class="d">按 <b>address of record</b> 判定，通常自动适用，无需主动联系 IRS</div></div></div><div class="pt"><span class="no num">02</span><div><div class="h">一年两个里程率</div><div class="d">7 月 1 日起，商用里程每英里 <b>72.5¢ → 76¢</b>，2026 要分段算</div></div></div><div class="pt"><span class="no num">03</span><div><div class="h">别只看联邦</div><div class="d">州税<b>不一定</b>跟随联邦延期，各州 DOR 要单独查</div></div></div></div>`),
    CARD('6 / 6', `<div class="big">对做税务的人来说，“deadline” 不是一个固定日子，而是一张<u>一直在动的地图</u>。</div><div class="tip"><div class="h">给同行提个醒</div><div class="m">手上有受灾地区客户的，排期前先逐个核对 <b>address of record</b> 与适用的新截止日。</div></div><div class="close-foot"><div class="cta">收藏这条，报税季少踩一个坑 ↓</div><div class="foot">美国报税不漏DDL</div></div>`),
  ]
  const labels = ['封面（图1）','数据（图2）','此刻延期的州（图3）','覆盖哪些申报（图4）','3 个要点（图5）','收束（图6）']
  return css + '\n' + cards.map((c, i) => `<div class="shot-label">▽ ${labels[i]}</div>\n${c}`).join('\n') + '\n'
}
function years0(d){ const ys=Object.keys(byYear).map(Number); return Math.min(...ys) }
