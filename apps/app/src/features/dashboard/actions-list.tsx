...
import { SeveritySectionHeader, severityToTier, type SeverityTier } from './severity-section'
...
function ActionsTableRow({
  ...
  // Why-now factors surface INLINE on Critical rows only. Other
  // tiers stay quiet — the eye should land on Critical context
  // first, not parse caption sublabels on every row.
  const whyNowFactors = tier === 'critical' ? topPriorityFactors(row) : []
          ...
          <Tooltip>
            ...
            <TooltipContent>
              <div className="flex max-w-[300px] flex-col gap-1 text-left">
                ...
                <span>
                  <Trans>
                    Your top 10 deadlines due this week, ranked by Smart Priority and bucketed
                    into Critical (act today), High (this week), and Upcoming. Subgroups inside
                    each tier separate work you can start now from work waiting on the client.
                  </Trans>
                </span>
              </div>
            </TooltipContent>
