...
export function AlertReadinessChip({
  readiness,
  firmImpact,
}: {
  readiness: PulseApplyReadiness
...
export function AlertDecisionStatusNotice({ alert }: { alert: PulseAlertPublic }) {
  ...
  return (
    <Alert variant={variant}>
      <AlertTitle className="flex flex-wrap items-center gap-2">
        <AlertReadinessChip readiness={readiness} firmImpact={alert.firmImpact} />
        {alert.firmImpact === 'no_current_match' ? (
