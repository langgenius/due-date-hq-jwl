...
import { AlertTriangleIcon, ArrowUpRightIcon, CheckIcon, EyeIcon } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <TableCell className="py-2 text-right">
        {/* "What obligations did this rule actually produce?" — the
            inverse of the obligation drawer's rule citation. Without
            this the rule library is read-only metadata and CPAs have
            to take the rule's word for what it triggered. Stop
            propagation so the row's "open rule detail" handler
            doesn't intercept. */}
        <Link
          to={`/obligations?rule=${rule.id}`}
          onClick={(event) => event.stopPropagation()}
          onKeyDown={(event) => event.stopPropagation()}
          aria-label={t`View obligations generated by ${rule.title}`}
