...
// Readiness "Docs N/M · missing X" signal — a primary triage column.
//
...
function ReadinessIndicator({
  ...
  //   • Partial  (0 < attached < total) → tertiary (gray). The count
  //     itself ("1/3") already names the gap and the AlertCircleIcon
  //     trailing flags incompleteness; a loud warning yellow would claim
  //     more weight than a "partial" state warrants in a scan. Quieter
  //     gray lets the row's other signals (due-date, status, action
  //     verb) lead.
  ...
  return (
    <span
      ...
      // Spell out the ratio on hover — it's source documents attached vs.
      // expected for this filing, so a CPA scanning the column knows what the
      // count tracks.
      title={t`${attached} of ${total} expected source documents attached for this filing`}
    >
      {/* No per-doc dots — the "Docs N/M" count already names the gap, and the
      ...
      {/* No "· missing N" tail — it's redundant, the user can do the
          subtraction "1 of 3 = 2 missing" instantly, so the row reads
          cleaner. The destructive/warning tone on the count itself already
          carries the urgency. */}
