...
import { MorningSweepProvider, useMorningSweep } from '@/features/pulse/MorningSweepContext'
...
export function RulesPulseRoute() {
  ...
  return (
    /* MorningSweepProvider wraps the entire shell so its context flows
       ...
       AND the embedded `<PulseChangesTab>` (where AlertsListPage reads
       `active` to override its filter state). Without the wrapper, the
       button could read state but couldn't influence the alerts list,
       or vice versa. */
