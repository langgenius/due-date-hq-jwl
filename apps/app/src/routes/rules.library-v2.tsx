...
export function RulesLibraryV2Route() {
  const { t } = useLingui()
  ...
  const switchToList = useCallback(
    (filter: RuleLibraryFilter, jurisdiction?: string, entity?: string) => {
      ...
      void navigate(`/rules/library-v2?${params.toString()}`)
    },
    [navigate],
  )
  ...
  return (
    <RulesPageShell title={t`Rule library (legacy)`} compact={inReview}>
      <div className="flex flex-col gap-3">
        <CoverageSummaryStrip onDrillIn={(filter, jur) => switchToList(filter, jur)} />
        <SourcesSummaryStrip />
        {/* Back-to-canonical link — the unified library lives at
            /rules/library now (was the preview, promoted 2026-05-21).
            This page (`/rules/library-v2`) is kept as a legacy
            reference for the toggle-driven matrix view. */}
        <div className="-mt-1 flex items-center justify-end">
          <Link
            to="/rules/library"
            className="inline-flex items-center gap-1 text-xs text-text-tertiary hover:text-text-accent hover:underline"
          >
            <Trans>← Back to Rule library</Trans>
          </Link>
        </div>
...
function CoverageSummaryStrip({
  ...
  return (
    <SummaryStrip
      ...
      detailHref="/rules/library-v2?view=rules"
