...
/**
 * InputGroup wraps Input/Textarea so the wrapper owns the border/bg state
 * layers and the inner control is `bg-transparent`. Pattern adapted from
 * Dify's `<input wrapper><icons /><input /></input wrapper>` shape.
 */
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-sm font-medium text-text-tertiary select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-sm [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
