...
const notices = JSON.parse(fs.readFileSync(new URL('./disaster-notices.json', import.meta.url), 'utf8'))
...
const section = (title, body) =>
  `<div style="font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#98A2B3;font-weight:500;margin:26px 0 12px">${title}</div>` + body
...
html += fresh.length
  ...
  : section('New this week', `<p style="margin:0;font-size:13px;color:#667085">No new IRS disaster-relief postponements in the last ${DAYS} days.</p>`)
...
if (soon.length) text += `COMING DUE WITHIN 30 DAYS (${soon.length})\n${soon.map(tLine).join('\n')}\n\n`
if (rest.length) text += `ALSO LIVE (${rest.length})\n${rest.map((n) => `- ${n.abbr} · ${n.event} — ${n.deadlineLabel} · IRS ${n.code}`).join('\n')}\n\n`
...
console.log(`wrote ${base}.html and .txt — review, then send to the subscriber list. Nothing was sent.`)
console.log(`suggested subject: IRS deadline changes this week — ${fresh.length ? fresh.map((n) => n.abbr).join(', ') + ' new' : 'no new relief'} · ${dateLabel}`)
