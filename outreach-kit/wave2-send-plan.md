...
- **Append-only guardrail:** `.outreach-state.json` must never shrink. A CI check
  (`.github/workflows/outreach-state-guard.yml` → `scripts/check-outreach-state.mjs`) blocks any PR
  that would drop recorded touches — so a stale copy can't revert the record and cause double-sends.
