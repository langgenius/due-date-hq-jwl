...
**Date:** 2026-06-21. Decision: make the `production-v2.html` exploration the **delivered marketing homepage**, built **into `apps/marketing`** as Astro components, and pull the rest of the marketing site up to its style. Balance the two core stories (v2's _insight_ hook + the existing site's product _depth_).

## Locked decisions

- **Target:** `apps/marketing` (Astro). Bring v2's _design_ into the existing app — inherit pricing infra, i18n, SEO, analytics. Do **not** rebuild infra or ship the standalone HTML.
...
- **Share primitives** with the product UI (the navy/cyan/scales/fonts). production-v2 _hardcoded_ `--accent: #2E368C` — that's literally the shared `--color-util-colors-primary-600`; replace hardcoded hex with token refs.
