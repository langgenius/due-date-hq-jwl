...
async function zh(d) {
  ...
  if (d.kind === 'correction') {
    seg.push(
      `【${d.source.org} 更正】${d.title[0]}${d.title[1] || ''}：${d.oldDate} → ${d.newDate}`,
    )
  ...
  if (d.kind === 'multi') {
    seg.push(
      (d.rows || []).map((r) => `${r.name}（${r.scope}）→ ${r.date}`).join('\n') +
...
async function en(d) {
  ...
  if (d.kind === 'correction') {
    seg.push(`Correction: our last post gave ${d.oldDate}. The ${org} notice says ${d.newDate}.`)
  } else if (d.kind === 'pending') {
    seg.push(
      `${org} has announced relief but has not set a date yet. ` +
        `The original ${d.oldDate} deadline still stands until it does.`,
    )
  } else if (d.kind === 'multi') {
    seg.push(`${org} postponed deadlines across four states — and the dates are not the same.`)
  } else {
    seg.push(
      `The ${org} postponed ${unshout(d.dateLabel || 'filing deadlines')}s ` +
  ...
  if (d.kind === 'multi') {
    seg.push((d.rows || []).map((r) => `${r.name} (${r.scope}) — ${r.date}`).join('\n'))
  }

  seg.push(`Notice ${d.source.noticeId}. ${detectedLineEN(d)}`)

  const tags = (d.caption?.tags || [])
    .slice(0, 3)
    .map((t) => `#${t}`)
    .join(' ')
  return { body: seg.join('\n\n'), tags, lede: seg[0] }
