...
的一键全量同步；只有导入事实足够支撑时才生成义务。没有明确标记 fiscal year 的导入客户按
calendar year 兜底；明确标记 fiscal year 但缺少 fiscal year end 的客户会进入 `Needs fiscal year end`
客户事实状态，不创建占位 deadline。缺少 entity type、state、tax type、payment status 或 extension
status 的记录仍按对应流程进入 review 或补事实。
...
3. Step 2 `AI Mapping`：
   ...
   - 如来源包含 tax year/fiscal 信息，可映射到 `Tax year type` 和 `Fiscal year end`。
   - 关注低置信度、降级提示和错误行列表。
...
5. Step 4 `Dry-Run Preview`：
   ...
   - 明确 fiscal year 但缺少 fiscal year end 的客户显示为 `Needs client facts`，CPA 需要到
     Client detail 补齐后再生成 deadline。
...
Obligations 是事务所的义务队列。它以表格方式显示客户、负责人、州/县、tax type、internal deadline、days、exposure、readiness、evidence 和 status，支持筛选、排序、批量更新、保存视图、导出和快捷键。Internal deadline 按 practice profile 中的 offset 从 statutory base deadline 提前生成；详情页同时显示税务机关规则来源中的 Filing Deadline、Payment Deadline 和用于推导 deadline 的 Tax period。
...
7. 单行处理：
   ...
   - 修改 readiness。
   - 点击 Evidence 打开证据侧栏。
   - 单击行打开 detail drawer 查看义务详情。详情顶部显示 internal deadline、税务机关规则来源中的 Filing Deadline / Payment Deadline，以及 CPA-facing Tax period；详情支持 Readiness、Extension、Risk、Evidence、Audit 五个 tab。
   - 在 Readiness tab 查看自动准备的 checklist，也可重新生成或编辑 checklist，发送/撤销客户资料
     请求，并复制公开 portal link。
   - 在 Extension tab 保存内部 extension plan；必须填写 decision memo，且内部 target date
     不能为空、不能晚于 official filing deadline。保存会把义务状态标记为 extended，但不会改
...
Rules 的 Pulse Changes tab 展示与当前 firm 客户匹配的 Regulatory Pulse。用户可查看政府来源变化、受影响客户、AI 置信度、结构化字段，并决定 apply、dismiss、snooze、revert 或 reactivate。
...
2. 使用 status filter 筛选 all、active、applied、partially applied、dismissed、reverted、snoozed。
3. 使用 source filter 按来源筛选。
4. 点击 alert 的 `Review` 打开详情侧栏。
5. 在详情中检查：
   - 来源、source URL、confidence。
   - AI summary 和 structured fields。
   - affected clients table。
   - low-confidence 或 source revoked 警告。
6. 对需要人工确认的客户进行勾选确认。
7. Owner/Manager 可使用基础 Pulse 操作：`Apply to # clients`、`Dismiss`、`Snooze 24h`、
   `Undo (24h)`、`Reactivate / Re-apply`、`Copy client email draft`。
8. Apply 成功后会写 audit、链接 evidence，并排队 digest。
...
Rules Console 展示 DueDateHQ 当前使用的规则覆盖、权威来源、Pulse Changes、Temporary Rules 和 obligation preview。它主要用于解释规则从哪里来、哪些规则 active、哪些需要人工审核、哪些政府来源变更影响客户、哪些临时 exception 正在改变客户截止日，以及给定客户事实会生成哪些义务。
...
2. `Rule Library`：
   - 默认展示 Coverage map，查看 active rules、pending review、sources watched、jurisdictions。
   - 点击 summary 里的 active / needs review 数字会筛选 Coverage map，而不是进入独立 Rule List。
   - 展开 jurisdiction 后，可在同一处查看 active / pending rule 摘要和引用来源。
   - 点击 `Review pending rules` 进入 pending review queue；点击 rule 打开右侧 detail。
   - 在 detail 中查看 applicability、due-date logic、extension、review reasons、evidence、practice review。
   - pending queue 中可勾选允许批量审核的规则，点击 `Review selected`，先看 bulk preview，再填写 batch review note。
   - 点击 `Accept selected` 后，只有成功项变为 active；source-defined rules 和 source_changed tasks 必须逐条审核。
3. `Sources`：
   - 按 watch 状态筛选 all、watched、paused。
   - 按 jurisdiction 筛选来源。
   - 点击行或外链图标打开官方来源。
4. `Pulse Changes`：
...
6. `Obligation Preview`：
   - Annual rollover 面板可选择 source / target filing year 和可选 client filter。
   - 点击 `Preview` 查看将创建、需要补客户事实、需要 review、duplicate、缺 active rule、缺 due date 的结果。
   - 点击 `Generate` 后只会为目标 filing year 有 active practice rule 且有具体 due date 的行创建 obligations；reminder-ready 行进入 `pending`，requires-review 行进入 `review` 并进入 Obligations triage，`Needs client facts` 行不会创建占位 obligation。
   ...
   - 点击 `Run preview`。
   - 查看 reminder-ready obligations、needs-client-facts items 和 requires-review items。
...
4. 与 Pulse ingest metrics/runbooks 一起使用，排查 failed snapshots、failure counts 和 source diagnostics。
