...
| AI 角色  | AI/parser 只能抽取模板草案或 source signal，不能让规则进入生产，不能直接改客户 deadline。                                   |
...
| 消费 API                                 | 5 个            | `rules.listSources`、`rules.listRules`、`rules.coverage`、`rules.draftConcreteRule`、`rules.previewObligations` |
...
- `source_defined_calendar` 只能表示 source-backed candidate 或 legacy active 待复核状态；不能直接 bulk accept，也不能显示为绿色 active coverage。
- Source-defined candidate 的 `Accept rule` 表示接受 AI 生成、用户审阅过的 concrete rule draft；AI draft 只写 `ai_output`，practice accept 后才写 active rule。
...
- pending template 可进入 preview 和 Rules 表的 Needs review 视图，但不能写入 obligations、reminders 或 risk。
