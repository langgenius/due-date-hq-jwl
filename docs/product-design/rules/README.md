...
| AI 角色  | AI/parser 只能抽取模板草案、review-only Pulse Alert 或全局 concrete draft；不能让规则进入生产，不能直接改客户 deadline。    |
...
| Pulse source watch                       | 1 个 Worker 流程 | cadence + 每周治理扫描生成 source snapshot；source 语义变化进入 Pulse overlay/review-only 流程，而不是内部 proposal                                                       |
...
- Source watch 只负责 freshness/snapshot；source 语义变化进入 CPA-facing Pulse Alerts。只有产品开发者决定把临时变化吸收进基座并发布新/变更 rule version 后，catalog sync 才 fan-out review task 并 enqueue 当前版本 AI concrete draft。
