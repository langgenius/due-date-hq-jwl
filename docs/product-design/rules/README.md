...
| 用户提醒 | 只基于 active practice rule 生成的 obligation 发送 30 / 7 天提醒。pending review 只用于预览和审核队列。                     |
...
| Source health                            | 1 个命令         | `pnpm rules:check-sources`；机器 source watch、Pulse source signal 与 practice review task 分开报                                                                         |
| Pulse source watch                       | 1 个 Worker 流程 | cadence + 每周治理扫描生成 source snapshot 或 source signal；source 语义变化进入 Pulse overlay/review-only 流程，而不是内部 proposal                                      |
...
- Source watch 只负责 freshness/snapshot/source signal；source 语义变化进入 Pulse。只有产品开发者决定把临时变化吸收进基座并发布新/变更 rule version 后，catalog sync 才 fan-out review task 并 enqueue 当前版本 AI concrete draft。
...
- `rules:concrete-drafts:*` 人工 report/inspect/backfill/snapshot CLI 已退役。concrete draft
  generation 只作为 catalog sync 发布当前 rule version 后的系统内部队列能力；source 变化、临时公告
  和语义差异进入 Pulse overlay/review-only 流程。
