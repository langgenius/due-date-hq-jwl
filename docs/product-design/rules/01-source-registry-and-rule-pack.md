...
Worker scheduled gate (Monday 09:00 UTC)
  -> rule.registry.source.reconcile queue item per RuleSource
  -> source freshness + R2 snapshot
  -> schema-validated AI analyzer proposal
  -> product developer review report
  -> manual edit to packages/core/src/rules/index.ts
  -> deploy catalog sync
  -> practice review task fan-out + current-version concrete draft prewarm
```

这个流程只把 source 变化转成内部 proposal，不直接改客户可见 rule。`html_watch` 和
`pdf_watch` source 会自动抓取、归档 R2、更新 freshness；`manual_review`、
`email_subscription` 和 `api_watch` source 会生成 `manual_check_due` proposal，不伪装成机器已核验。
source 内容未变化时只更新 freshness，不触碰 AI concrete draft。source 内容变化后，内部 analyzer
把结果分类为 `no_rule_change`、`existing_rule_update` 或 `new_rule`；`no_rule_change`
proposal 会被保存为 dismissed operational record，不触发 draft 失效。

产品开发者 review proposal 后，才手动更新 `packages/core/src/rules/index.ts`：

- 只更新 evidence retrieved/reviewed 时间，不 bump `rule.version`。
- due date、applicability、extension policy、source mapping 或 source-defined calendar 语义变化，必须 bump 旧 rule version 或新增 rule。
- 发布后 catalog sync 对新增 rule 创建 `new_template` review task；对已存在 rule 的版本变化，只给已经 active 或历史 review 过该 rule 的 practice 创建 `source_changed` task。
- 新版本出现时旧版本 open task 会标为 `superseded`，但不会自动把旧 active practice rule 改成 `pending_review`。
