...
| 剩余 states + `DC` seed | 29      | 0                     | 53                | 只登记 tax-topic / filing FAQ / statute / due-date 级官方 source；未补齐的 domain/entity 继续显示 source gap，不生成 placeholder rule                                              |
...
当前代码 registry 合计 189 个官方 source。Federal source 已从单一 Pub 509 + 7004
...
`IL`、`MA`、`NJ`、`PA`、`NC`、`VA`、`AZ`、`CO`、`MI`、`OH`、`OR`、`SC`、`TN`、
`UT`、`WI`。
...
`UT` 对无 required quarterly individual estimated tax schedule 使用 `not_applicable`。其它州仍按已登记的精确 source 生成 review-only candidate，缺 source 的领域只显示 gap。
...
`GA`、`IL`、`MA`、`NJ`、`PA`、`NC`、`VA`、`AZ`、`CO`、`MI`、`OH`、`OR`、`SC`、
`TN`、`UT`、`WI`)
