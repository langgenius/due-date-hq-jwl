...
estimated-tax template 只在同一 source 明确覆盖 estimated payments 时生成。Business
candidate 只在登记了明确 business source seed 的 jurisdiction/domain 生成：当前包括 CA
business due dates、NY tax calendar、TX franchise、FL CIT due dates、WA excise due
dates；不会复用个人所得税 source 生成 business rule。Fiduciary、PTE/composite、
sales/use、withholding 和 UI wage report 必须先登记精确到领域的官方 source，之后才能生成
source-backed template；系统不会再用 broader tax agency 或 UI/workforce homepage 顶替。
