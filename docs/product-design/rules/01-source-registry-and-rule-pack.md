...
| 剩余 states + `DC` seed | 45      | 0                     | 82                | 只登记 tax-topic / filing FAQ / statute / due-date 级官方 source；未补齐的 domain/entity 继续显示 source gap，不生成 placeholder rule                                              |
...
当前代码 registry 合计 108 个官方 source。Federal source 已从单一 Pub 509 + 7004
...
样板州；第一批补齐的 completed source packs 为 `AL`、`CA`、`NY`、`TX`、`FL`、`WA`。
其中 `TX`、`FL`、`WA` 对无州级 income/withholding 的 domain/entity cell 使用
`not_applicable`，不会登记不存在的 source，也不会生成 placeholder rule。其它州仍按已登记的
精确 source 生成 review-only candidate，缺 source 的领域只显示 gap。
