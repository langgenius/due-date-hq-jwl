...
> `CapsFieldLabel` (`components/primitives/caps-field-label.tsx`) with a `variant` prop:
...
> now on `CapsFieldLabel`. Batches 1–2 did the safe surfaces (**65 sites / 23 files**);
> batch 3 the detail drawers + first straggler pass; batch 4 the remainder found
> by a broadened audit (dev-logs `fieldlabel-migration-batch3` / `…-batch4`):
>
> - **Detail drawers** (batch 3): `AlertDetailDrawer`, `ObligationQueueDetailDrawer`,
>   queue `panels`.
> - **App-wide stragglers** (batch 3 + 4, ~25 sites): clients
>   (detail/peek/summary/title-switcher), migration wizard, `BlockerContextCard`,
>   `CompletedKeyDates`, `splash`, `SurfaceSummaryStrip`, `StatBand`,
>   `BulkConfirmList`, `audit-event-drawer`, `deadlines-at-a-glance`,
>   `DeadlineNavigatorRail`, dashboard `actions-list`, alert
>   `PulseFormRevisedCard`/`AlertStructuredFields`, `workload-page`,
>   `ShortcutHelpDialog`, `billing`, and the first real `<label htmlFor>` form
>   label (`generation-preview-tab`, enabled by the htmlFor passthrough).
>
> ⚠️ **Audit method (learned in batch 4):** find candidates by **color + size**
> (`uppercase` + `text-text-tertiary|secondary|muted` + a small size), NOT by
> tracking class — batch 3's tracking-specific grep missed labels with no tracking
> or arbitrary tracking values (and the subagent mis-bucketed a few as "values").
> The color+size grep is the comprehensive one.
>
> Decisions made during the sweep (Yuqi):
>
> - **Table column headers** keep `<TableHead>` (+ its width/align classes) and
>   wrap the content in `<CapsFieldLabel variant="group">` — they do NOT become a
>   bare CapsFieldLabel.
> - **`<h3>`/`<h2>` heading-labels** that were really small-caps labels (not prose
>   titles) demote to `as="div"` CapsFieldLabels. **Exception:** a caps label that is
...
>   "AI trace") uses `as="h2|h3|h4"` — CapsFieldLabel renders the heading tag with the
...
>   the date-picker weekday grid) stays as-is — it is not a discrete label.
...
>   `tabular-nums` counts, `text-column-label`-token labels (already canonical), the
>   `DueDateHQ` **wordmark** "HQ" (brand, not a label), and the onboarding/migration
>   **"Step N of M"** progress eyebrows (deliberate wide `tracking-[1.4px]`, kept by
>   Yuqi as an intentional exception).
>
...
> wider tracking; and `CapsFieldLabel` forwards native attrs (`htmlFor`, `id`,
