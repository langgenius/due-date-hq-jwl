...
- **Selected（2026-06-21 更新 · SUPERSEDES the accent-tint spec above）**：Yuqi 决定改用 **实心 accent pill**——active 行现在是 `bg: state-accent-solid` + `text: text-inverted`（白字白 icon），count badge 也翻白（inventory：`text-inverted` on active；urgent：保留 active 时的红 pill）。Acme 参考的 register。承接 `accent-tint` 的 calm 取舍换成更明确的"当前页"实心锚点；chroma 仍在 container（实心填充）而非 colored-text-on-tint。落在 `SidebarMenuButton` variant（`packages/ui/src/components/ui/sidebar.tsx`）。
...
> - **2026-06-22 filter-dropdown 词汇统一 sweep**（cross-surface cohesion，Yuqi）：所有过滤/排序下拉收敛到 **pill 词汇**。① 新 **`SingleSelectFilter`** primitive = 单值过滤 + 排序的唯一长相（见下表）。② `FilterTrigger` 新增 **`count`**（consolidated "Filters" 的统一计数徽章）+ **`size="sm"`**（rail 紧凑变体，取代手搓 className）。③ **/audit** 从 native `<Select>` 表单控件全面改为 pill（Category·Range·Filters·Clear），不再是工具条里的"表单"。④ consolidated 触发器 label 统一 **"Filters"**（复数；/deadlines 旧 "Filter" 已改）、统一 SlidersHorizontalIcon + count 徽章，**删除** /deadlines 的 border/weight/hideChevron override 与 /alerts·/audit 的 `text-base` override。⑤ active 一律 = **accent-hover 底色**（不靠 border / text-color shift / weight）。⑥ rail Sort/Status + /alerts Morning-sweep chip 不再手搓——走 primitive / 对齐 pill spec。**live-vs-apply 例外**：小 popover 即时生效；只有 /deadlines 560px 多 facet sheet（含 tab + saved views）保留 staged Apply/Reset（一次提交 5 个 facet 值得显式 commit）——这是**记录在案的例外**，不是 drift。**待办**：/alerts List/Map toggle 在 PageHeader + 工具条各有一个（高度不一），是 IA 取舍待 Yuqi 决定是否去重。详 `filter-dropdown-cohesion-2026-06-22.md`。
> - **小型大写 label（Register B）** → **`CapsFieldLabel`** (`primitives/caps-field-label`)，`variant`：`field`（B2，12px/medium/tracking-eyebrow，value/field 标签）· `group`（B1，11px/semibold/tracking-eyebrow-tight，group band / 列头）。`text-text-tertiary` + uppercase 内置。**禁止**手搓 `text-caption-xs/text-xs ... uppercase tracking-* text-text-tertiary`——legacy 迁移已**全部完成**（2026-06-17，batch 1–4，含 detail drawer 与 app-wide stragglers；以 color+size 而非 tracking 审计为准）。2026-06-18 把 `FieldLabel`→`CapsFieldLabel`（49 sites）以消除与表单族 **`FieldLabel`** (`ui/field`，form-control label) 的同名碰撞——caps eyebrow 永远是 `CapsFieldLabel`，`ui/field` 的 `FieldLabel` 专指输入控件 label。详 `section-header-style.md`。
...
| 过滤下拉触发器（pill chrome）                    | **`FilterTrigger`** (`patterns/filter-trigger`)                                               | h-9 rounded-full（`size="sm"`=h-7，给 narrow rail，**勿**再用 className 手改高度）。active = accent-hover **bg only**（**禁止** per-caller border / font-weight / text-base override——active 靠底色，不靠边框）。`count` prop = consolidated "Filters" 触发器的 accent-solid 计数徽章（统一 /deadlines·/alerts·/rules·/audit 的 "N filters on" 长相）。clear-filters = **always-rendered ghost + `disabled` when empty + label "Clear filters"**                                                                                                               |
| 单值过滤 / 排序选择器（one-value filter, sort）  | **`SingleSelectFilter`** (`patterns/single-select-filter`)                                    | 2026-06-22 新增。`FilterTrigger` pill + `DropdownMenuRadioGroup` body，收敛各处手搓的 "DropdownMenu + render FilterTrigger + RadioGroup"。`Label │ Value ⌄` 两段式（label=过滤什么，accent value=当前值）；options 支持 `icon`+`count`+`triggerLabel`（短值，如 menu "Newest first" → pill "Newest"）；`size="sm"` 给 rail。/audit Category·Range、/alerts·/deadlines Sort by、rail Sort 全走它。**勿**用 native `<Select>` 做工具条过滤（/audit 旧三处已收敛）                                                                                                |
...
| obligation 状态 mark（the status glyph）         | **`StatusMark`** (`obligations/status-control`) → **`StatusRing`** (`primitives/status-ring`) | 2026-06-18 取代 per-status lucide 图标集。progress-ring：happy path 环逐步填满（not_started 虚线→in_review ~50%→filed ~85%→completed 实心+勾），off-path（waiting/blocked）用 pause/slash 打断环。`currentColor` 单色——hue 来自 `STATUS_ICON_COLOR`/`_ON_PILL`。`StatusMark` 接 raw `ObligationStatus`（内部 `STATUS_RING_LEVEL` 映射）。**不是** SeverityChip（那是严重度）、不是 BadgeStatusDot                                                                                                                                                              |
| 严重度/优先级/风险 level chip（URGENT/HIGH/MED） | **`SeverityChip`** (`primitives/severity-chip`)                                               | 2026-06-18 新增。soft-tint `--severity-*` ramp：`level` `critical`(红)/`high`(橙)/`medium`(琥珀)/`neutral`(灰)。alert priority、dashboard severity、rule risk 全走它。**颜色只编码紧急度/严重度**——客户影响面这种"另一根轴"的事实标签用 `level="neutral"`（绝不双重报警）。`shape` `pill`(默认)/`square`(registry eyebrow)，`size` `md/sm`。**不是** status pill（那是 Badge）、不是 AI-confidence pill、不是 status dot                                                                                                                                       |
...
| 变更 before→after diff（旧→新值）                | **`ValueDiff`** (`primitives/value-diff`)                                                     | 2026-06-19 新增（灵感 img-153）。一个 regulatory 变更本质是 diff——due date 推移、form 被取代、jurisdiction 重指派。`mode` `inline`(默认：旧值删除线 + 箭头 + 新值 + toned delta，/alerts KeyChange 行) / `compact`(紧凑 `旧 › 新` chip，full diff + delta 在 hover tooltip，给 audit-log/activity 等窄行)。delta 的色调由调用方传 `deltaClassName`（如 `DUE_DATE_DIFF_TONE_CLASS[dueDateDiffTone(n)]`）——sooner=红/later=绿 的单一来源仍在 `due-date-tone.ts`，**色只编码 delta**（绝不整体上色，遵守 no-double-highlight）。曾在 `PulseAlertRow` 手搓，已收敛 |
...
| 380px master-list 侧栏                           | **`ListRail`** + `ListRailHead/Title/Section/Body` + `useRailArrival` (`patterns/list-rail`)  | 统一 width/border/padding/scroll chrome；内容各 rail 自管。**到达行为**（2026-07-02）：行组件必须用 `useRailArrival(active)` —— rail 首绘时选中行 `scrollIntoView('start')` 置顶 + 播一次 `animate-arrival-wash`（globals.css：accent-active-alt 起色 → 落到 state-base-hover 常驻选中色），回答"我点的就是这条吗"；rail 内点击/↑↓ 翻页只 `'nearest'` 滚动、**永不**重播 wash（用户自己点的目标无需确认）。accent 允许出现在这里正因为它是瞬态——常驻选中仍是 neutral fill（勿改）。alerts / deadlines-navigator / deadlines-in-page 三个 rail 共用             |
