...
| **后台任务**            | **Cron Triggers** + **Queues**（email / pulse / dashboard / audit / social 五组 + DLQ） | Pulse ingest / Email outbox / dashboard 聚合 / audit 打包 / X 单条发布；X 调度零外部依赖。GitHub Actions 仅 best-effort 镜像 review draft，不参与调度；Cloudflare Workflows 尚未启用（wrangler.toml 无 workflows binding） |
