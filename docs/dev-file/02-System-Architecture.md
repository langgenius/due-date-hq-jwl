...
也路由到同一 handler——只 drain + 发 ops alert，不重跑失败 handler。
...
| `/api/ops/social/*`                | 手写 Hono route                                                                                | X 候选预览、审批、取消、unknown 对账                                                         | dev 开放；其余环境只认 `SOCIAL_OPS_TOKEN` bearer             |
