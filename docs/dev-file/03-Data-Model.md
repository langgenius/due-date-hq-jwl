...
| 字段                                                                  | 约束 / 语义                                                                                                                                                            |
| --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `channel / local_date / post_id`                                      | `UNIQUE(channel, local_date)` 是单个 ET 日期最多一条的数据库硬闸；自动调度另检查前两个 ET 日期的 run，以保证中间至少空两天；live post 另有 partial unique 防止跨日重复 |
...
自动补充 draft 不增加 schema 或未来预约 row。09:00 ET 分支读取前两个 ET 自然日是否已有
`social_publish_run`；任一天有占槽 run 都暂停当天 claim，使下一个自动槽位最早为三天后，但暂停日
仍执行 draft reconciliation。自动 scheduler 与一次性 `seed-drafts` 共享
`createDraftIfBufferBelow` 的原子 `INSERT ... SELECT ... WHERE draft_count < target` 路径，把当前仍
符合 Social candidate 条件的 draft 数补到目标值（自动固定为 3，operator 可选 1..14），而不是每次
额外追加目标数量；shadow claim 返回的 draft 在补充前重读并计入目标。写入前先清理 runtime-invalid
active row，逐候选条件插入，写入后重读同一 eligible queue projection，因此重复 Cron、网络超时重试
或并发请求既不能把 buffer 补过目标，也不会让隐藏的失效 draft 占用可见 review buffer。operator
backfill 可显式选择上线切点以前尚未 outbox 的最新 Alert；自动 scheduler 受
`X_SOCIAL_START_AT` 和三天 cadence 限制。两条路径都不自动 approve，也不预建未来
`social_publish_run`。
