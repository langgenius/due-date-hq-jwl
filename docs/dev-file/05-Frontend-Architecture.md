...
shadcn Sidebar（base-vega）打包了 3 种 collapse 模式（`offcanvas` / `icon` / `none`）+ `SidebarRail` + cookie 持久化 + `Cmd+B` 全局快捷键 + `floating` / `inset` chrome variant；这些**全部不在我们用例里**：DESIGN §5.4 固定自己的 sidebar geometry，`⌘K` 等全局快捷键由 app keyboard shell 显式登记，§6「borders before shadows」反对 floating/inset。**自建 ~200 行 vs 引入 730 行未用 API**。详见 `docs/dev-log/2026-04-27-app-shell-sidebar.md` 的决策矩阵。
...
| `SidebarMenuBadge` / `SidebarMenuBadgeDot`                   | sidebar count pill / collapsed dot | Expanded Numeric/Small + tabular-nums; collapsed rail renders a dot only                                                                                                                                 |
...
- **Practice identity 在 sidebar 顶部保持可见，但不暴露切换操作**（不是 PRD §3.2.6 原始的右上 dropdown）：当前 shell 只显示 active practice 的 monogram/name，不渲染 `Practices` popover、`Add practice` 或 `⌘⇧O` switcher hotkey。Practice profile / Members / Billing 继续承载事务所管理；内部 RPC 仍可沿用 firm。
