...
[vars]
PUBLIC_APP_URL = "https://app.due.langgenius.app"
...
- Marketing 只允许 `PUBLIC_*` 前缀的变量（Astro 唯一的客户端可见前缀，通过 `import.meta.env.PUBLIC_*` 暴露）。现有两个：`PUBLIC_APP_URL=https://app.due.langgenius.app`（CTA base）和 `PUBLIC_ENABLE_DEMO`（§2.3 Hero demo CTA 开关；dev 下恒开，staging/prod 需显式 `true` 且 server 侧 `ENABLE_PUBLIC_DEMO` 同步开启）。
...
`https://app.due.langgenius.app`）；`script-src` 含 `'unsafe-inline'`，因为 theme no-flash 与
