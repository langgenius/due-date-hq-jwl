...
| Final CTA | 转化                 | Open the workbench / Contact sales（`mailto:sales@duedatehq.com`）；不增加第三个 CTA                             |
...
SaaS app 的 `/billing/checkout?plan={solo|pro|team}&interval={monthly|yearly}`；页面带
Monthly/Yearly billing toggle（默认 monthly，纯 Astro `<script>` 切换 CTA 的
`data-href-monthly` / `data-href-yearly`，无 React island）。未登录用户由 app
自己的 auth/onboarding loader 接管后再回到 checkout。`/rules` 与 `/states/*` 已落地
（见下方 GEO 段）；新增公开内容仍不搭复杂 CMS。

GEO 公开内容已扩展为静态 Astro 页面，registry 集中在
`apps/marketing/src/lib/seo-content.ts`（typed dictionary，不引入 CMS）：

- `/rules` 总览 + 3 个 rule reference 页：
  `/rules/{form-7004-extension-deadline,s-corp-deadline-operations,partnership-form-1065-deadline}`；
- `/state-coverage` + 15 个州详情页 `/states/*`（首发 california / new-york / texas /
  florida / washington；后续补 illinois / new-jersey / pennsylvania / georgia /
  massachusetts / north-carolina / arizona / colorado / ohio / michigan）；
- 5 个 guide：`/guides/{cpa-deadline-risk,evidence-backed-tax-deadline-software,weekly-cpa-deadline-triage,migrate-cpa-deadlines-from-excel,extension-vs-payment-deadlines}`；
- 3 个 comparison 页：`/compare/{file-in-time-alternative,taxdome-deadline-operations,karbon-deadline-operations}`。

每个页面都有对应 `/zh-CN/*` 版本。内容只描述软件覆盖、官方来源处理、证据复核和运营边界；
禁止把公开页写成税务建议、法律建议或客户特定适用性判断。

Trust v1 将公开信任面补齐为静态 Astro 页面：`/about`、`/security`、`/privacy`、
`/terms`、`/status`（动态路由 `[trustPage].astro`，slugs 与文案注册在
`src/lib/trust-pages.ts`），每个页面都有对应 `/zh-CN/*` 版本。这些页面只说明产品边界、
...
OWNER-APPROVED UPGRADES` 右侧 mono 备注 + Monthly/Yearly segmented toggle，yearly 带
  `Save about 20%` badge）、**Plans Row**（2 列 tablet、4 列 desktop，cards stretch
  等高；每卡含一块 AI 面板：`aiLabel` pill + `aiDescription` + `aiFeatures` 列表）、
  **FAQ**（FAQ eyebrow + 24 px heading + 3 列 panel）。
...
- 价格走 token 区分：数字 (`$39` / `$79` / `$149`) 用 `font-mono font-bold text-[40px]`，
  ...
  Geist Mono 11 px `tracking-[0.06em]` `text-text-muted`。年付展示约 8 折的月均价
  （`$31` / `$63` / `$119` / `From $319`，billed yearly）+ `yearlySavings` 备注。
...
含 `priceKind: 'numeric' | 'text'`、4 条 `features`、月/年双价（`price` /
`yearlyPrice` + `yearlyCadence` / `yearlySavings`）、AI 面板（`aiLabel` /
`aiDescription` / `aiFeatures`）和 `hrefKind: 'checkout' | 'app' | 'contact'`
（checkout 套餐另带 `checkoutPlan`）；顶层自带 `plansHeader` / `billingToggle` /
`faqHeader`。en + zh-CN 必须同步更新。
...
apps/marketing
  Astro static site
  zero React islands today (@astrojs/react not registered; see §4 / §5.1)
...
apps/marketing/
├── astro.config.mjs
├── wrangler.toml                # Workers Static Assets（§7）
├── package.json                 # name: @duedatehq/marketing
├── public/
│   ├── _headers                 # 安全响应头（§7）
│   ├── favicon.svg
│   └── og/                      # home.en.png / home.zh-CN.png
├── src/
│   ├── pages/
│   │   ├── index.astro          # default locale landing
│   │   ├── pricing.astro
│   │   ├── rules.astro          # + rules/[rule].astro
│   │   ├── state-coverage.astro # + states/[state].astro
│   │   ├── guides/[guide].astro
│   │   ├── compare/[comparison].astro
│   │   ├── [trustPage].astro    # about / security / privacy / terms / status
│   │   ├── 404.astro
│   │   ├── robots.txt.ts        # + llms.txt.ts（均 prerender）
│   │   └── zh-CN/               # 上述全部页面的本地化镜像（含 zh-CN/404）
│   ├── components/              # Hero / Problem / Workflow / Proof / Security /
│   │                            # FinalCta / Pricing / TopNav / Footer /
│   │                            # PreferenceSwitcher / StructuredData / Geo*、State*、Trust* …
│   ├── layouts/
│   │   └── BaseLayout.astro
│   ├── lib/                     # site / app-url / seo-content / trust-pages /
│   │                            # structured-data / content-metadata …
│   ├── i18n/
│   │   ├── types.ts             # LandingCopy 契约
│   │   ├── en.ts
│   │   └── zh-CN.ts             # locale 常量来自 @duedatehq/i18n，不在本地重复
│   └── styles/
│       └── globals.css
└── tsconfig.json
```

Astro 默认不向页面发送 JS。当前站点**零 React island**（无 `islands/` 目录）：
PreferenceSwitcher、Pricing billing toggle、Workflow viewed 事件全部用原生 Astro
`<script>` 实现。React 组件只有在需要交互时才作为 island 加载并显式使用 `client:*`
directive；静态 section 优先写 `.astro`，不要把整个 landing 做成 React SPA。
...
- zh-CN 有自己的 404：`src/pages/zh-CN/404.astro` 构建为 `dist/zh-CN/404.html`。Cloudflare
  Workers Static Assets 的 `404-page` 模式按目录向上取**最近的** `404.html`（官方文档
  "the contents of the nearest `404.html` file"），因此 `/zh-CN/*` 未命中路径在生产环境
  得到中文 404，其余路径仍回落到根级 `/404.html`（英文页保持跨 locale 兜底）。注意
  `astro dev` 不模拟 nearest-404：dev 下任意未命中路径只渲染根级英文 404，直接访问
  `/zh-CN/404` 可预览中文页。
...
export default defineConfig({
  ...
  trailingSlash: 'never',
  build: { format: 'file' },
  // `@astrojs/react` 故意未注册：零 island 时注册它会向 dist/_astro/ 输出
  // ~190 KB 的 orphan React client bundle。引入真实 island 时再加回 react()
  // 并按 §5.1 重新核算 JS 预算。
  integrations: [
    sitemap({
      // 防止 i18n fallback 把隐藏的 /zh-CN/zh-cn/* URL 写进 sitemap。
...
依赖钉版（已写入 `pnpm-workspace.yaml` catalog，遵守 §3 of `01-Tech-Stack.md` 钉版政策）：

```yaml
# catalog: 实际条目（saveExact 锁定；版本以 pnpm-workspace.yaml 为准）
astro: <pinned>
'@astrojs/sitemap': <pinned>
'@astrojs/check': <pinned> # `astro check` 类型检查（devDependency）
# '@astrojs/react'：暂不在 catalog —— 首版零 React island；
# 引入真实 island 时再钉版加回（catalog 注释已写明该政策）。
```
...
/* Tailwind 4 source globs — keep in sync with the marketing src layout. */
...
import '../styles/globals.css' // globals.css 内部 @import @duedatehq/ui/styles/preset.css
---
...
└── src/
    ...
    │                       # LOCALE_LABELS / LOCALE_SHORT_LABELS（EN / 中 短标签），
    │                       # isLocale(), localeFromLanguageSignal()（纯函数，无 browser globals）
    ├── headers.ts          # LOCALE_HEADER = 'x-locale'
    └── index.ts            # barrel（无独立 detect.ts；浏览器侧 detect/persist
                            # 在 apps/app/src/i18n/locales.ts）
```
...
Cloudflare Workers Static Assets 的默认 HTML handling 会把历史 `/*.html` 请求用临时 `307`
规范化到无扩展名 URL。`apps/marketing/public/_redirects` 用相对路径规则把这类历史地址覆盖为
永久 `301`（含根页、一级页、二级页和 `/zh-CN/*`）；canonical 与 sitemap 始终只输出无扩展名
URL。该文件只处理站内 path 规范化，www → apex 与 HTTP → HTTPS 仍由 Cloudflare zone rule 负责。
...
primary、Hero demo（`Try a live demo`）、FinalCta primary）使用 **`<a href>` 同标签跳转**，**不**加 `target="_blank"`。理由：
...
- Marketing 只允许 `PUBLIC_*` 前缀的变量（Astro 唯一的客户端可见前缀，通过 `import.meta.env.PUBLIC_*` 暴露）。现有两个：`PUBLIC_APP_URL=https://app.duedatehq.com`（CTA base）和 `PUBLIC_ENABLE_DEMO`（§2.3 Hero demo CTA 开关；dev 下恒开，staging/prod 需显式 `true` 且 server 侧 `ENABLE_PUBLIC_DEMO` 同步开启）。
...
/*
  ...
  Content-Security-Policy: default-src 'self'; img-src 'self' data: https:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'; frame-ancestors 'none'
```

CSP 现状两点：`connect-src` 已按"首版不向 app 子域发请求"收紧到 `'self'`（未来要发请求时再放行
`https://app.duedatehq.com`）；`script-src` 含 `'unsafe-inline'`，因为 theme no-flash 与
Workflow / Pricing / PreferenceSwitcher 都依赖 `is:inline` / inline `<script>`。若以后要求严格
CSP，需迁移到 nonce/hash 方案并同步改 `_headers`。
...
- OG 图按 locale 各一张存在于 `public/og/`：`og/home.en.png`、`og/home.zh-CN.png`（1200×630），文档化在 §6.3 hreflang 旁。
- `sitemap.xml` 由 `@astrojs/sitemap` 自动生成（依赖 §4 `astro.config.mjs` 的 `site` + `i18n` 字段，自动输出 absolute URL）；app 子域不进入 marketing sitemap。
- `robots.txt` 由 `src/pages/robots.txt.ts` prerender，指向 `https://duedatehq.com/sitemap-index.xml`，并显式允许 Googlebot、OAI-SearchBot、GPTBot、ClaudeBot、Claude-SearchBot 和 PerplexityBot 访问公开站。
...
- 内容新鲜度使用固定 review 日期，不使用 build time。`src/lib/content-metadata.ts`
  维护 `CONTENT_PUBLISHED_ON`、`CONTENT_REVIEWED_ON`、per-slug 发布/复核日期和单州官方来源链接；页面可见
...
1. `https://duedatehq.com/*` 对匿名用户返回 `200 text/html`，不需要登录、cookie 或 JS 才能读取正文。
2. 每个可索引页面的 canonical、Open Graph、JSON-LD URL 和 sitemap URL 都使用同一个 HTTPS origin：`https://duedatehq.com`。
3. `https://duedatehq.com/robots.txt` 允许 Googlebot 抓取，并声明 `Sitemap: https://duedatehq.com/sitemap-index.xml`。
4. `https://duedatehq.com/sitemap-index.xml` 只列 marketing 公开 URL，不列 `app.duedatehq.com` 的登录后页面。
5. 英文和中文页面互相声明完整 `hreflang`，并提供 `x-default` 指向英文默认页。
6. 404 fallback 返回 HTTP 404，并带 `noindex, nofollow`，避免错误路径进入索引。
7. `https://app.duedatehq.com/robots.txt` 由 Vite public asset 输出
   `Disallow: /`，`apps/app/index.html` 带 `noindex, nofollow`，避免登录后 SPA shell
   被当作公开 SEO surface。

上线后要在 Google Search Console 中添加 `https://duedatehq.com` property，提交
`https://duedatehq.com/sitemap-index.xml`，并用 URL Inspection 检查首页、`/pricing`、
...
HSTS；如果 `curl -I http://duedatehq.com/` 不是 301/308 到 HTTPS，需要在 Cloudflare zone
...
- Redirect smoke：通过本地 Wrangler 验证历史 `/*.html` 返回 `301` 到无扩展名 canonical；
  canonical URL 本身返回 `200`
...
- Bundle budget：`dist/` 内单页 JS 总量 < 50 KB gz（CI 的 size-limit / 自写脚本**尚未落地**，目前靠零 island 架构 + review 把关；引入第一个 island 前必须先补上）

---

## 10. 实施顺序

1. 在 `pnpm-workspace.yaml` catalog 钉版加入 `astro` / `@astrojs/sitemap` / `@astrojs/check`（遵守 §3 of `01-Tech-Stack.md`，禁止字面量版本进入 `apps/marketing/package.json`；`@astrojs/react` 暂缓，见 §4）。
2. 新增 `apps/marketing` Astro static app，`astro.config.mjs` 完整声明 §4 中的 `site` / `trailingSlash` / `vite.plugins[tailwindcss()]` / i18n routing，接入 `@duedatehq/ui` preset（`@astrojs/react` 不注册，待首个真实 island 再加）。
...
- Astro React integration：`@astrojs/react` 在 `astro.config.mjs` 的 `integrations` 中注册（本项目当前**未**注册——零 island；此条留作未来引入 island 的参考）。
