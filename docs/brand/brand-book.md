...
*reference*, not the source of truth — the source of truth is the code:
...
obligation from *not started → filed → completed*.

**Mood:** precise · calm · dollar-aware · glass-box · keyboard-first.
**Lineage:** Ramp × Linear light workbench — *not* Notion warmth, *not* Stripe
gradient, *not* Bloomberg neon.
...
*中文版 → [logo-rationale.zh.md](logo-rationale.zh.md)*

Supplied by Yuqi (2026-06-16). A deliberately **graphic** mark — not a letterform.

### Why graphic, not a "D"
The product interior is dense and text-heavy — forms, dates, lists, tables. The brand
goes the opposite way: a simple geometric mark instead of a "D" monogram. Two intents:

- **Separate the brand from the product's "heavy text."** Logo = graphic, UI = text;
  each does its own job, and the brand layer gets room to breathe.
- **Soften the formality.** Deadlines and compliance are inherently serious and a little
  tense; a clean, friendly shape lets the brand feel lighter and more approachable.

### Form → meaning
- **Four stacked bars** = a stack of pending deadline items / filings — the rows of
  obligations the firm is holding.
- **The offset (third) bar** = a **newly inserted item** — a task that just dropped into
  the queue and hasn't aligned yet. And because it's new and not yet in line, it's also
  **the one asking for your attention**: the two readings are cause → effect, the same
  beat told twice.
  - This rhymes with the product itself: the highlight cyan `--color-brand-highlight`
    `#14C5F6` is the in-app "new / unseen" color (unread dots, new alerts). So the mark's
    offset bar and the product's cyan markers speak one language — **new = look here.**
- **App-icon form** = ivory bars (`--color-brand-ivory`) on a navy rounded square
  (`--color-brand-ink`); navy + ivory only, no accent in the mark. On `/splash` the
  square drops away so the bars stand on their own.
- **HQ** stays in the *wordmark* as a quiet sans tag — the command center, not the mark.

### Status — v1
A **first pass**, intentionally restrained and shipped as a placeholder / foundation.
The direction is set — graphic-first, non-letterform, stack + insertion — but the visual
is still light; weight, the offset amount, and any motion/accent are open for the next
iteration.
...
*for CPA firms* tagline. Serif = trust; the sans tag signals software.
...
- **Why two accent tiers** — navy is the calm default, cyan is the loud exception. One
  ...
  we split them.
- **"Color only for risk"** — gray is the safe baseline; color carries business meaning
  (risk / status) only, never decoration. The screen stays calm; only what matters gets color.
- **Why the two layers coexist** — the identity navy is fixed (who we are); the product runs
  the calm Dify workbench palette (how the tool works). Keeping them separate stops the brand
  navy from bleeding into the product as an accent.
...
- **Hierarchy needs two changes:** shift *both* token and weight (`text-xl/600` title
