...
/deadlines                         → Deadlines queue ★ this PRD
/deadlines/<short-ref>             → Queue with right-side drawer open
...
See: obligations generated by this rule (read-only list)
       │
       ▼
Click obligation → navigate to /deadlines/<short-ref>
```
...
External calendar (Google / Outlook) shows obligations as events
       │
       ▼
Click event → opens /deadlines/<short-ref> in a browser tab
```
