...
| **Filing Obligation** (§9.2) | client, jurisdiction, authority, form, tax year, obligation type (filing\|payment\|deposit\|information\|client_action\|internal), due date rule, calculated due date, adjusted due date, extension available, extension form, payment due separate, source, confidence/review status | `ObligationInstancePublic` covers most | ⚠️ `due_date_rule` (the *formula*) isn't stored — we store the calculated date; ⚠️ `payment_due_separate` flag missing; ⚠️ `confidence/review_status` on the rule itself missing |
