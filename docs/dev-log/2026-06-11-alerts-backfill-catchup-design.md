...
**⚠️ 唯一剩余步骤（需 owner 执行——staging 创建 secret 属共享基础设施变更，agent 无授权）**：

```bash
# ① 给 staging worker 设置 ops token（一次性）：
cd apps/server && openssl rand -hex 24 | tee /tmp/ops-token.txt | pnpm exec wrangler secret put E2E_SEED_TOKEN
# ② 触发回填入队（精确命中那 24 条快照）：
curl -X POST https://app.due.langgenius.app/api/ops/pulse-backfill \
  ...
  -d '{"sourceIds":["irs.disaster","irs.guidance","irs.newsroom","ny.dtf.press","fl.dor.tips","ca.cdtfa.news"]}'
# 期望响应 {"queued":24,...}。幂等：失败重跑安全。
# ③ 验收（必须为 0 —— 硬闸门保证 backfill 的 deadline_shift 都解析出了原 due date）：
pnpm exec wrangler d1 execute due-date-hq-staging --remote --command \
 "SELECT COUNT(*) FROM pulse p JOIN pulse_source_snapshot s ON s.pulse_id=p.id \
  WHERE s.ingest_method='backfill_seed' AND p.action_mode='due_date_overlay' \
  AND p.parsed_original_due_date IS NULL AND p.status='approved'"
```

注：staging 目前 0 个 firm，回填只生成全局 pulse 池；新 firm 注册导入客户时由首批 obligations catch-up 自动物化成 band。州级回填 launch 前按 cohort 州逐源 POST 同一路由即可（cap 200/次）。
