...
注：staging 目前 0 个 firm，回填只生成全局 pulse 池；新 firm 注册导入客户时由首批 obligations catch-up 自动物化成 band。州级回填 launch 前按 cohort 州逐源 POST 同一路由即可（cap 200/次，1212 条 baseline 积压待用）。7. `feat(alerts): retire the Already-in-effect band`（4ec5fd59，**owner 展示层修订**）——catchup 行不再用独立紧凑分组带，改用与新闻流相同的 PulseAlertRow 卡片、同一条 50 上限查询（无 origin 过滤），按 `isActiveAlert` 正常落 Review/Active tab。state-not-news 语义完全保留在后端（不计 new、不发邮件、注册时静默物化）；选中批量忽略弹窗的「保护窗口 60 天内关闭」警示自然覆盖 catchup 卡。10 条 band 专属 msgid 经 clean-worktree extract --clean 移除（834a0854），zh 缺翻译归零。8. `feat(ingest) + feat(alerts) grounding`（9116db1e + b30b7bea，**Kemp 错链事故的根治**）——①摄入层：announcement 链接条目（HTML+RSS）带 `enrichFromUrl`，ingest 循环用 dedupe 身份哈希先判新旧（顺带消灭重复条目每轮的 R2 重归档），仅对全新条目抓详情页替换索引摘录、并从详情恢复真实发布日（每轮上限 10、baseline 期不抓、politeness 生效）；source_url 从此指向条目页而非 hub。②抽取层两道确定性闸门（全路径生效、降级不丢弃）：日期溯源——四个承重日期（orig/new due、effective_until、protective deadline）必须能以美式公告的任一写法在原文定位，否则 quarantined + `pulse.extract.grounding_quarantined` 指标；无日期 overlay 一律 quarantined（永不匹配且永不过期的死行，Apply-readiness approve-anyway 行为退役）。被隔离件可由更完整的再观测经 dedupe fold 晋升或另立带日期新件。excerpt-guard 失败维持原 retry-sweep 重驱（不属于丢弃；为防幻觉引文晋升不转 quarantine）。**存量清理（owner 一条命令）**：`pnpm --dir apps/server exec wrangler d1 execute due-date-hq-staging --remote --command "UPDATE pulse SET status='rejected' WHERE status='approved' AND action_mode='due_date_overlay' AND parsed_original_due_date IS NULL AND parsed_new_due_date IS NULL"`（4 条：Kemp/GA、tx.franchise_extensions、wa.dor.news、ca.cdtfa.news）。9. **存量链接全量审计（2026-06-11，67 条 approved）**：真坏 4 条——`37ab9694`(MA 列表页)、`56090172`(UT 列表页)、`15230349`(WI 列表页)、`76d55767`(SC **text/href 错配**：标题文字配到了 W-2 上传页的 href；今日实测索引页 anchor 正常，属 6/2 抓取时一次性解析错位；同事件已有正确孪生 `sc.income_tax`/01284919)。疑似 9 条中 5 条实为正当（ca.ftb_541_booklet / ny.nyc_yonkers / tx.pir_oir 等注册页本身即内容页）+ 3 条 drift/source_status 类本应指向被监控页。清理命令（owner 执行）：`UPDATE pulse SET status='rejected' WHERE id IN ('37ab9694-0c15-4734-854a-8ddc54b7a315','56090172-8f61-4b54-bd72-52490a25f923','15230349-c972-4f56-8644-8a719b447c2e','76d55767-de34-4856-8c31-b9ec6c059347')`。错配类未来防护：enrich 抓错误 href 的正文 → AI 判 no_regulatory_change 或日期无法溯源 → 永不 approve。10. **「正确监听」三连修（1ed411a5 + 29adc55f + golden-audit，2026-06-11 晚）**：①「131 个 stale 源」诊断反转——95% 是设计内低频节奏（120×14d/7×90d/3×30d 规则参考页）+ email-only（oh/fed.irs_newswire）+ 被 API 适配器替代的索引页；真缺陷仅 2：fl.dor.tips 的 R2 同 key 重复 put 触发 429（archivePulseRaw 改为全量 HEAD 预检）、ak 整站 robots 禁爬被 15min 热重试 86 次（robots 失败改 7 天退避；AK 暂无网页覆盖，可寻替代源或 email 订阅）。②prompt v5：「变化 vs 复述」DECISION TEST（带实测踩坑样例）+ extract.ts 确定性兜底（overlay 的 summary+引文无变化语言 → quarantined，metric restatementOverlay）——v4 时代 67 条 approved 仅约 14 条真变化。③黄金集周审计：每周一 10:00 UTC 无条件重抓 6 个高风险源（irs.disaster/irs.newsroom/GA/SC/ca.ftb.tax_news/ny.dtf.press）对照快照表，漏报即 ops 告警含条目清单；注册表漂移由常驻测试钉死。AI 额度已由 owner 充值，453 条 credits 积压将由 retry sweep 在解锁后自动消化（14 天窗口内）。11. **销账（2026-06-11 晚核实）**：①§7 的 8 个州源种子实际已于 09:06 UTC 处理完毕（owner 的 curl 成功过）——8/8 带 ai_output_id：7 条 AI 判 no_regulatory_change → ignored，1 条（wv.income_tax）historical_policy_dates 正确抑制，0 新 pulse，结果合理，无遗留。②存量坏链清理完成：4 条坏链行（MA/UT/WI 列表页 + SC href 错配 76d55767）已 rejected + dedupe_key=NULL（agent 执行，owner 明确授权）；§8 的 4 条无日期 overlay 此前已被 owner 的 UPDATE 处理。验证：无日期 approved overlay = 0，SC 正确孪生（01284919）仍 approved。③充值生效旁证：approved 总数 67 → 73（净增 10+，retry sweep 在消化积压）。Alerts 模块所有已知存量问题至此全部闭环。
