...
once the smooth-scroll _settled_ — so the marker lagged a beat behind the click.
...
**Latent bug caught:** the `AlertsNotificationsBell` was rendered _only_ in the
...
the locked-4-tab _label_ per the fix-all request; tab count/structure unchanged.)
