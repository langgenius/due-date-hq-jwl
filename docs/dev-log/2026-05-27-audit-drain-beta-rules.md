...
wrapped each branch's return string. Updated the sole call site (line 1301) to pass `t` along.
...
unwrapped template literal. Added`useLingui`hook and wrapped with`t\`…\``.
