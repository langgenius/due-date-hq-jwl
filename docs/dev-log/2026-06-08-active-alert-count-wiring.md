...
Yuqi: "you need to wire them correctly." After unifying the count chip's *look*,
the *numbers* still disagreed — the sidebar nav badge showed 8, the /alerts header
...
- `routes/alerts.tsx` header and `AlertListRail` head now call it (removed the
  ad-hoc `matched` filters; the rail's Unresolved *tab* still filters rows).
