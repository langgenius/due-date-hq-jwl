...
- **Radius "drift" (rounded-lg vs rounded-xl).** This is the _documented
  two-tier system_: `rounded-xl` (12) = standalone card (Card primitive
...
- **"Redundant" row ⋯ menu.** Peek is a hidden ⌘-click gesture (no visible
  Eye icon); the menu's "Quick peek"/"Open detail" are the _discoverable_
...
currently mounted the stripe — this is a _preventive_ removal from the
...
- First tried a primitive-level accent background wash
  ...
  _selected_ — too heavy, fought the restraint rule. Dropped it; the dot
  alone is cleaner.
