...
running the lenses *fresh* surfaced real **correctness / a11y / i18n bugs** the
...
- **Error lie-states (correctness):** `/today` Priorities and the client **Work tab**
  showed the green "All clear / No deadlines yet" state on a *failed* load (queries
...
- Notification toggle: real `onMutate` optimism (it previously *claimed* optimism with none).
