...
- **Most remaining richness is data, not design** — this demo rule is a sparse
  _candidate_ (no clients/obligations, no concrete due date, single source, no
  ...
  shows a _populated_ rule; the component already renders the richer treatments
...
- **Title pinned (was missing).** The hero (`RuleDetailHeroCard`) was the first
  _scrolling_ card, and Base UI's open-focus scrolled it away — the panel opened
