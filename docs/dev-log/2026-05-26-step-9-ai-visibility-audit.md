...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). Added `origin: 'ai' | 'manual'` + `ai_generated_at` + `user_edited_at` columns (migration `0030_ai_provenance_actor_type.sql`). `AiProvenanceBadge` primitive renders the Astroid marker on `origin === 'ai' && userEditedAt === null` rows; marker drops automatically via F-022 + audit-events via F-023. Used existing `source` column for template/custom (orthogonal axis); the new `origin` is the AI-vs-manual axis.
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). The canonical pattern is implemented in `readiness.updateChecklistItem`: the procedure passes `dropsAiOrigin: true` when the input touches a value field (label / description / note); the repo enforces the precondition (`previousOrigin === 'ai'`) and stamps `user_edited_at`. Status-only changes (mark received) do NOT drop the marker — they're confirmations, not overrides. Future surfaces (rules concrete drafts, structured fields) should follow the same shape.
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). Implemented as `previousActorType` on `audit_event` + a distinct `readiness.checklist_item.ai_overridden` action string. The audit drawer surfaces an "Overrode AI suggestion" warning chip when `previousActorType === 'ai' && actorType === 'user'`. Repo emits the override event on the same call path as the marker drop (F-022).
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). Migration `0030` added `actor_type` (NOT NULL DEFAULT 'user') + the `idx_audit_firm_actor_type_time` index. Audit log table renders a dedicated Astroid tile for autonomous AI rows and an "AI-assisted" chip for human-applied-AI-value rows. Audit list contract supports the `actorType` filter with an `'ai_any'` convenience bucket. The drawer filter-dropdown UI for actor_type is the one piece still pending — schema and contract are ready for whoever wires that.
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). Closed with F-035. Both the log table and the drawer resolve `event.actorType === 'ai' ? 'AI' : (actorLabel ?? actorId ?? 'System')`, so autonomous AI rows read as "AI" not "System".
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). `ai_event_metadata_json` column (nullable) added to `audit_event`; `AiEventMetadataSchema` defines the shape (model, prompt_version, input/output tokens, latency_ms, guard_status, confidence, ai_output_id) — every field optional so the disclosure surface grows without contract bumps. The audit drawer renders a dedicated "AI trace" section for AI / AI-assisted events; gracefully shows "Trace metadata not recorded for this row" for events that pre-date the column.
...
- Status: SHIPPED (2026-05-27 — η audit-drain pass). The `AiProvenanceBadge` primitive's tooltip surfaces "Generated by AI" + the timestamp + the F-022 override convention ("Edit this item to confirm the value. The AI marker will drop."). Wired into `ChecklistItemRow`.
