...
// Category-page explainer, rendered under the tool cards. Only the deadline
// category has one: at 448 words it was the thinnest page on the site, and it is
// the one category where GSC shows real long-tail intent within reach
// ("cpa due date tracking software" pos 18.8, "due date tracking software for
// accountants" 25, "tax deadline tracking software" 24.5) alongside a vocabulary
// the site never used at all ("tax calendar software", "tax compliance calendar
// software" — 22 impressions at position 70–78). One deeper page, not a new one.
const catEssay = {
  monitor: [
    '<h2 class="revh2">Tax calendar, due date tracking, deadline monitoring — the three different things these tools do</h2>',
    '<div class="toolsection"><p>The terms get used interchangeably in vendor copy, but they name three separate jobs, and a firm usually needs a specific one. Buying the wrong layer is how firms end up with a tool that is technically accurate and still lets a date slip.</p></div>',
    '<div class="usecase"><h3>The tax calendar — what is due, and when</h3><p>A tax calendar is the statutory schedule itself: which return or deposit is due on which date, for a given entity type and jurisdiction. The IRS publishes the federal version annually as Publication 509, <em>Tax Calendars</em>; each state revenue department maintains its own. On its own a calendar is a reference document — it knows nothing about your clients.</p></div>',
    '<div class="usecase"><h3>Due date tracking — which of my clients has what coming up</h3><p>Due date tracking applies that calendar to a client list: every client × obligation × period becomes a record with an owner, a status, and an extension path, and it rolls forward each year instead of being rebuilt each January. This is what a passive tracker like File In Time does, and what the due-date module inside Karbon, Canopy, TaxDome, Financial Cents, or Jetpack Workflow does for firms already running one of those.</p></div>',
    '<div class="usecase"><h3>Deadline monitoring — what just changed, and who does it hit</h3><p>Monitoring is the active layer on top: watching whether the statutory dates themselves move. They move more often than the annual-calendar mental model suggests — IRC §7508A lets Treasury postpone filing and payment deadlines after a federally declared disaster, and a single FEMA declaration can shift dates for every client in the covered counties at once. A tracker holds whatever dates were entered; a monitor detects the change and identifies the affected clients. DueDateHQ sits in this layer; ONESOURCE Calendar covers the enterprise, multi-entity end of the same problem.</p></div>',
    '<h2 class="revh2">What to ask before you buy</h2>',
    '<div class="toolsection"><ul>' +
      '<li><strong>Which jurisdictions does it actually maintain?</strong> Federal dates are stable and published. State dates are where coverage claims differ, and where the gaps are.</li>' +
      '<li><strong>Who maintains them, and how fast?</strong> A mid-season change only helps if it reaches the tool before the deadline does.</li>' +
      '<li><strong>Does a changed date reach your client list, or just a newsletter?</strong> The useful output is the list of affected clients, not the announcement.</li>' +
      '<li><strong>Do you already own this?</strong> If your practice management software tracks due dates well enough, the standalone spend buys you monitoring, not tracking.</li>' +
      '<li><strong>Can dates leave the tool?</strong> Calendar feed, export, or API — so the schedule is usable outside the vendor that stores it.</li>' +
      '</ul></div>',
  ].join('\n'),
}
...
const cats = [
  ...
  },
  {
    ...
    n: 3,
    // GSC 07-07→08-01: the whole "tax calendar software" / "tax compliance
    // calendar software" cluster hit this site at position 70–78 while the exact
    // phrase "tax calendar" appeared nowhere on it. Title and description now
    // carry the vocabulary the cluster actually uses.
    title: 'Tax Calendar & Due Date Tracking Software (2026) — CPA Field Guide',
    desc: 'Tax calendar, due date tracking, and deadline monitoring software for CPA firms: File In Time, DueDateHQ, and ONESOURCE Calendar — what separates a compliance calendar from a passive tracker from an active monitor.',
  },
  {
    key: 'pm',
...
const faqByCat = {
  ...
  monitor: [
    ...
    [
      'What is tax calendar software?',
      'A tax calendar is the statutory filing schedule itself — which return is due on which date for a given entity type and jurisdiction. The IRS publishes the federal version annually as Publication 509, Tax Calendars; each state agency maintains its own. Tax calendar software turns that schedule into per-client, per-period records instead of a PDF you read once a year.',
    ],
    [
      'What is the best due date tracking software for accountants?',
      'It depends on which of the three jobs you are buying. File In Time is the long-standing passive tracker for solo and small firms; ONESOURCE Calendar is the enterprise compliance calendar for multi-entity work; DueDateHQ is the active monitor that watches the IRS, states, and FEMA for date changes and flags the affected clients. Firms already on Karbon, Canopy, TaxDome, Financial Cents, or Jetpack may not need a standalone tracker at all.',
    ],
    [
      'Does due date tracking software cover state deadlines?',
      'Coverage varies and it is the question worth asking hardest. Federal dates are stable and published; state dates change more often, and a mid-season change only helps you if it reaches the tool before the deadline does. Ask any vendor which jurisdictions it maintains, who maintains them, and how a change made in March reaches your client list.',
    ],
  ],
  pm: [
    [
...
const tools = [
  ...
  ['Keeper', 'https://doublehq.com', 'Practice Management'],
...
for (const c of cats) {
  ...
  const body = [
    ...
    crumb,
    section,
    c.key === 'monitor' ? catEssay.monitor + '\n\n' + monitorDefLink : '',
    '</main>',
    catFaqHtml,
    sibnav,
    guideNav,
