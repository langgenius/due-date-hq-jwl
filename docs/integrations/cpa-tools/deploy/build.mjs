...
const _detailShots = assetMap("detail-shots"); // real product screenshots, shown only on tool detail pages
...
const LABEL_MAP = {
  "File export": "Export only", "Closed": "Export only",
  "No public API yet": "No API yet", "Zapier": "Zapier only", "Gated": "Gated API",
  "Partner": "Partner API", "Partner program": "Partner API", "API · partner": "Partner API",
  "Suite · gated": "Gated API",
};
const normLabels = (h) =>
  h.replace(/(<span class="tag"><span class="dot d-[a-z]+"><\/span>)([^<]+?)(<\/span\s*>)/g,
    (m, a, l, z) => a + (LABEL_MAP[l.trim()] || l.trim()) + z);
const whiteLogo = (h) => h.replace(/(<div class="logo(?:[^"]*)?")\s+style="[^"]*"/g, '$1 style="background:#fff"');
...
for (const c of cats) {
  ...
  (sec.match(/<article class="card"[\s\S]*?<\/article>/g) || []).forEach((card) => {
    ...
    toolData.push({
      ...
      openLabel: g(/<span class="tag"><span class="dot d-[a-z]+"><\/span>([^<]+?)<\/span\s*>/).trim(),
      dataSeg: g(/data-seg="([^"]+)"/),
...
function head(title, desc, canonical) {
  const fav = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'><rect width='30' height='30' rx='8' fill='%231A1A1A'/><path d='M15 4.2 L17.6 12.4 L25.8 15 L17.6 17.6 L15 25.8 L12.4 17.6 L4.2 15 L12.4 12.4 Z' fill='%23FFFFFF'/><path d='M12.4 12.4 L15 4.2 L17.6 12.4 Z' fill='%232F6DA6'/></svg>";
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
for (const c of cats) {
  ...
  const catFaqHtml = catFaq.length
    ? `<div class="faq"><div class="wrap"><h2>${c.label.replace("&", "&amp;")} — FAQ</h2>` +
      catFaq.map((f) => `<details class="qa"><summary>${f[0]}</summary><p>${f[1]}</p></details>`).join("") + `</div></div>`
    : "";
...
for (const t of toolData) {
  ...
  const tfaqHtml = `<div class="faq"><div class="wrap"><h2>${esc(t.name)} — FAQ</h2>` + tfaq.map((f) => `<details class="qa"><summary>${esc(f[0])}</summary><p>${esc(f[1])}</p></details>`).join("") + `</div></div>`;
  const graph = {
    "@context": "https://schema.org",
    "@graph": [
      org,
  ...
  const dshot = _detailShots[t.slug];
  ...
  const body = [
    ...
    `<p class="toollede">${esc(t.desc)}</p>`, facts, connects, screenshot, "</main>", tfaqHtml, related, method, footerBlock, revealScript, ld,
  ].join("\n\n");
...
function guidePage(slug, title, h1, intro, groups, faq) {
  ...
  const ld = `<script type="application/ld+json">\n${JSON.stringify(graph, null, 2)}\n</script>`;
  const faqHtml = faq ? `<div class="faq"><div class="wrap"><h2>FAQ</h2>` + faq.map((f) => `<details class="qa"><summary>${f[0]}</summary><p>${f[1]}</p></details>`).join("") + `</div></div>` : "";
...
function vsPage(A, B) {
  ...
  const faqHtml = `<div class="faq"><div class="wrap"><h2>${esc(A.name)} vs ${esc(B.name)} — FAQ</h2>` + faq.map((f) => `<details class="qa"><summary>${esc(f[0])}</summary><p>${esc(f[1])}</p></details>`).join("") + `</div></div>`;
...
function altPage(T) {
  ...
  const faqHtml = `<div class="faq"><div class="wrap"><h2>${esc(T.name)} alternatives — FAQ</h2>` + faq.map((f) => `<details class="qa"><summary>${esc(f[0])}</summary><p>${esc(f[1])}</p></details>`).join("") + `</div></div>`;
...
const statsUrl = ORIGIN + "/cpa-software-statistics";
{
  ...
  const faqHtml = `<div class="faq"><div class="wrap"><h2>FAQ</h2>` + faq.map((f) => `<details class="qa"><summary>${esc(f[0])}</summary><p>${esc(f[1])}</p></details>`).join("") + `</div></div>`;
  const graph = { "@context": "https://schema.org", "@graph": [ org,
