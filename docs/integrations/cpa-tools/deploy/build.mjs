...
function head(title, desc, canonical) {
  const fav =
    "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'><rect width='30' height='30' rx='8' fill='%231A1A1A'/><path d='M15 6 L18.2 15 L11.8 15 Z' fill='%2322936A'/><path d='M15 24 L11.8 15 L18.2 15 Z' fill='%237C7C77'/></svg>"
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
for (const c of cats) {
  const secOpenRe = new RegExp(
    `(<section class="section" data-cat="${c.key}">\\s*<div class="cat-head">)(<h2>[\\s\\S]*?<\\/h2><span class="count">[^<]*<\\/span>)(</div>)`,
  )
  homeBody = homeBody.replace(
    secOpenRe,
