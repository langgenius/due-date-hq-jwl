...
const logoAssets = assetMap('logos')
const shotAssets = assetMap('shots')
const slugify = (s) =>
  s
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
fullBody = fullBody.replace(/<article class="card"[\s\S]*?<\/article>/g, (card) => {
  ...
  if (logoAssets[s])
    card = card.replace(
      ...
      `<img class="asset logo-img" src="logos/${logoAssets[s]}">`,
    )
  if (shotAssets[s])
    card = card.replace(
      '<img class="asset shot-img">',
      `<img class="asset shot-img" src="shots/${shotAssets[s]}">`,
    )
  return card
})
...
function head(title, desc, canonical) {
  const fav =
    "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'><rect width='30' height='30' rx='8' fill='%231A1A1A'/><path d='M15 6 L18.2 15 L11.8 15 Z' fill='%232F6DA6'/><path d='M15 24 L11.8 15 L18.2 15 Z' fill='%237C7C77'/></svg>"
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
homeBody = homeBody.replace(
  /<span class="clabel">Category<\/span>[\s\S]*?<span class="sep" aria-hidden="true"><\/span>\s*/,
  '',
)
// add "view all" links from each homepage category heading to its page
for (const c of cats) {
  const secOpenRe = new RegExp(
    `(<section class="section" data-cat="${c.key}">\\s*<div class="cat-head">(?:<span class="cat-icon">[\\s\\S]*?<\\/span>)?)(<h2>[\\s\\S]*?<\\/h2><span class="count">[^<]*<\\/span>)(</div>)`,
  )
  homeBody = homeBody.replace(
    secOpenRe,
    `$1$2<a class="cat-more" href="/${c.slug}">View all ${c.n} &rarr;</a>$3`,
  )
}
...
for (const c of cats) {
  ...
  const catFaqHtml = catFaq.length
    ? `<div class="faq"><div class="wrap"><h2>${c.label.replace('&', '&amp;')} — FAQ</h2>` +
      catFaq.map((f) => `<div class="qa"><h3>${f[0]}</h3><p>${f[1]}</p></div>`).join('') +
      `</div></div>`
    : ''
  const guideNav =
...
for (const t of toolData) {
  ...
  const tfaqHtml =
    ...
    tfaq.map((f) => `<div class="qa"><h3>${esc(f[0])}</h3><p>${esc(f[1])}</p></div>`).join('') +
    `</div></div>`
  const graph = {
    '@context': 'https://schema.org',
    '@graph': [
...
function guidePage(slug, title, h1, intro, groups, faq) {
  ...
  const faqHtml = faq
    ? `<div class="faq"><div class="wrap"><h2>FAQ</h2>` +
      faq.map((f) => `<div class="qa"><h3>${f[0]}</h3><p>${f[1]}</p></div>`).join('') +
      `</div></div>`
    : ''
  const body = [
    topbar,
...
function vsPage(A, B) {
  ...
  const faqHtml =
    ...
    faq.map((f) => `<div class="qa"><h3>${esc(f[0])}</h3><p>${esc(f[1])}</p></div>`).join('') +
    `</div></div>`
