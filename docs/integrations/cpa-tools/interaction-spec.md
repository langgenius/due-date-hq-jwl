...
- **No dead ends**: the count tells you *why* the list is shorter; the empty state (if reached) names the situation.
...
| **Scroll-spy vs filter** | a `display:none` section returns `getBoundingClientRect().top = 0`, which passes the `≤ line` test → could mark a *hidden* section active | 🔴 **G1 bug** — fix below |
