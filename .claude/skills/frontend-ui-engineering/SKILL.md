...
When this skill is used in DueDateHQ, read the root `CLAUDE.md` and `AGENTS.md` first. Their coding
and architecture rules supersede generic examples below; in particular, do not introduce React
`useEffect`, semicolon-oriented formatting, or alternate component organization that conflicts with
the repository's established vertical structure.
